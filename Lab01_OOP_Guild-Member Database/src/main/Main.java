...
public class Main {
	...
	public static boolean addDepartment(String newName) {
		...
		try {
			...
			if (myDatabase.isExists(newName)) {
				...
				System.out.println("the department has been created");
				return true;
			}
		} catch (EmptyGuildNameException emptyguild) {
			System.out.println(emptyguild);
		} catch (DuplicateGuildNameException duplicateguild) {
			System.out.println(duplicateguild);
		}
		return false;
	...
	public static void removeDepartmentFromDatabase(int index) {
		...
		ArrayList<GuildMember> removedDepartment = myDatabase.removeDepartment(index);
		getUnassignedDepartment().addMultipleMembers(removedDepartment);
		System.out.println(
				"Department removed. " + removedDepartment.size() + " member(s) added to unassigned department.");
