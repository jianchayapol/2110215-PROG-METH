...
public class TestMainCharacter {
	...
	public void testSwitchCharacter() {
		...
		assertEquals(250, player.getCurrentLifePoint());
		assertEquals(100, player.getAttack());
		assertEquals(80, player.getDefense());

		player.setCurrentLifePoint(120);
		MainCharacterCard testMainChar = new MainCharacterCard("Test MainCharacter ", "test MainCharacter", 200, 100,
				80, 0.2f);

		testMainChar.switchCharacter(player);

		assertEquals(200, player.getMaxLifePoint());
		assertEquals(96, player.getCurrentLifePoint());
		assertEquals(100, player.getAttack());
		assertEquals(80, player.getDefense());
	}
	...
	public void testSacrifice() {
		...
		assertEquals(137, player.getCurrentLifePoint());

		player = new Player("testPlayer", 285, 50, 30);
		testMainChar = new MainCharacterCard("Test Main Character", "test maincharacter", 200, 190, 90, 0.3f);
		player.setCurrentLifePoint(180);
		testMainChar.sacrifice(player);
		assertEquals(215, player.getCurrentLifePoint());
	...
	public void testLevelUp() {
		...
		// Add Level 3 case
//		testMainChar.levelUp(player);
//		assertEquals("Test Main Character [level 3]", testMainChar.getName());
//		assertEquals(432, player.getMaxLifePoint());
//		assertEquals(172, player.getAttack());
//		assertEquals(138, player.getDefense());
